    ...
    for (i = 0; i < signals.length; i++) {
      ...
      switch (sig.type) {
        case 'message':
          ...
          if (!execs[sig.left]) execs[sig.left] = starts[sig.left] = getID('exe');
          if (sig.right && !execs[sig.right]) execs[sig.right] = starts[sig.right] = getID('exe');
          html += renderer.execution(lives, execs, starts);
          if (sig.line == 'reply') {
            if (sig.dir == 'left') execs[sig.right] = false;
            else if (sig.dir == 'right') execs[sig.left] = false;
          }
          break;
        case 'note':
