(function() {
  ...
  var rules = {
    ...
    sd_message: /^ *(x) *(<(?:-+|=+|\.+)|(?:-+|=+|\.+)>) *(x)? *$/,
    sd_note: /^ *(x)? *\[ *(x(?: *, *x)*)? *] *(x)? *$/,
    sd_fragment_start: /^ *(fragment) *\[ *(x(?: *, *x)*) *] *$/,
    sd_fragment_end: /^ *\/(fragment) *$/,
    sd_guard: /^ *\[ *: *(.+)]? *$/,
    _x: /[^\[\]\-:=.<> ]+/,
    _fragment: 'ref|alt|opt|par|loop|break|critical|assert|neg|ignore|consider'
  };
  rules.sd_message = replace(rules.sd_message)
  (/x/g, rules._x)
  ();
  rules.sd_note = replace(rules.sd_note)
  (/x/g, rules._x)
  ();
  rules.sd_fragment_start = replace(rules.sd_fragment_start)
  (/x/g, rules._x)
  (/fragment/g, rules._fragment)
  ();
  rules.sd_fragment_end = replace(rules.sd_fragment_end)
  (/fragment/g, rules._fragment)
  ();
  ...
  marked.Lexer.prototype.sequence = function(cap) {
    ...
    for (i = 0; i < signals.length; i++) {
      ...
      if (cap = this.rules.sd_fragment_start.exec(sig)) {
        ...
        signals[i].on = minOf(split);
        signals[i].size = maxOf(split) - signals[i].on + 1;
        continue;
      }

      if (cap = this.rules.sd_guard.exec(signals[i])) {
        signals[i] = {
          type: 'guard',
          desc: split[1]
        };
      ...
      if (i > 0) {
        split = signals.splice(i, 1)[0];
        signals[--i].desc += '\n' + split;
      } else {
        console.log('unmatch', signals[i]);
      }
    }
    return {
      type: 'sequence',
  ...
  marked.Parser.prototype.sequence = function(token) {
    ...
    for (i = 0; i < signals.length; i++) {
      ...
      switch (sig.type) {
        case 'message':
          html += message(sig, body, cols);
          break;
        case 'note':
          html += note(sig, body, cols);
          break;
        case 'fragment_start':
          stack.unshift({name: sig.name, on: sig.on, size: sig.size, guards: 0});
          html += fragment_start(stack[0], body, cols);
        ...
        case 'guard':
          if (stack.length) {
            html += guard(stack[0], body, cols);
            stack[0].guards++;
          }
          break;
        case 'fragment_end':
          if (stack.length && stack[0].name == sig.name) {
            html += fragment_end(stack[0], cols);
            stack.shift();
          }
          break;
      }
      html += padding(len, stack);
    }
    while (stack.length) {
      html += fragment_end(stack[0], cols) + padding(len);
      stack.shift();
    }
    html += '</tbody></table></div></div>\n';
    return html;

    function fragment_start(frag, body, cols) {
      var start = frag.on * 2 + 1
        , size = frag.size * 2
        , content = '<div class="fragment fragment--'+frag.name+'"><div><span>'+body+'</span></div></div>';
      return row(start, size, content, cols);
    }
    function guard(frag, body, cols) {
      var start = frag.on * 2 + 1
        , size = frag.size * 2
        , content = '<div class="fragment-guard fragment--'+frag.name+' '+(frag.guards ?'bordered':'')+'"><span>'+body+'</span></div>';
      return row(start, size, content, cols);
    }
    function fragment_end(frag, cols) {
      var start = frag.on * 2 + 1
        , size = frag.size * 2;
      return row(start, size, '<div class="fragment-end fragment--'+frag.name+'"></div>', cols);
    }
    function padding(actors) {
      return '<tr class="padding">' +
        new Array(actors + 2).join('<td colspan="2"></td>') +
        '</tr>';
    }
    function message(sig, body, cols) {
      var start = sig.left * 2 + 2
        , size = Math.max((sig.right - sig.left) * 2, 1)
        , content = '<div class="message '+sig.line+' '+sig.dir+'"><span>'+body+'</span></div>';
      return row(start, size, content, cols);
    }
    function note(sig, body, cols) {
      var start = sig.on * 2 + (sig.side == 'left' ? 0 : sig.side == 'over' ? 1 : 2)
        , size = Math.max(sig.size * 2, 1)
        , content = '<div class="note">'+body+'</div>';
      return row(start, size, content, cols);
    }
    function row(start, size, body, cols) {
