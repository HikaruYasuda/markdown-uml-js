(function(undefined) {
  ...
  extend(Actor, Part, {
    toString: function() {
      if (this.name == this.alias) return this.name;
  ...
  Diagram.prototype = {
    ...
    puts: function(line) {
      ...
      if (m = /\s*> ([\s\S]+)/.exec(line)) {
        return this.actor(this.prev = new Actor(m[1]));
      }
      if (/^\s*$/.exec(line)) {
        this.prev = null;
      }
      if (this.prev instanceof Actor) {
        return this.prev.put(this.prev + line);
      }
  ...
  (function() {
    ...
    var check = '<input type="checkbox" class="task-list-item-checkbox"> ',
      checked = '<input type="checkbox" class="task-list-item-checkbox" checked="checked"> ';
    renderer.listitem = function(text) {
      if (/^\s*\[[x ]]\s*/.test(text)) {
        text = text.replace(/^\s*\[ ]\s*/, check).replace(/^\s*\[x]\s*/, checked);
